...
\section{正規部分群と剰余群}
	...
	\end{dfn}
	\begin{example*}
		$n$次交代群$\mathcal{A}_n$は$n$次対称群$\mathcal{G}_n$の正規部分群となる．
	\end{example*}
	\begin{dfn}
	...
	\subsection{群$G$への共役類分解}
		...
		\paragraph{共役類の性質}
			\begin{itemize}
				...
				\item 可換群は，各元が単独で共役類．
			\end{itemize}
			\begin{example*}[$\mathcal{G}_3$の共役類]
				\[\mathcal{G}_3=\qty{e,c,c^{-1},\sigma_1,\sigma_2,\sigma_3}\]
...
\section{準同型定理}
	...
	\begin{thm}[準同型定理]
		...
		による，群の同型\redunderline{$G/\Ker f\simeq\Im\qty(f)$}が成り立つ．この$\bar{f}$を\redunderline{$f$から誘導される(引き起こされる)同型写像}という．
	\end{thm}
	\begin{align*}
		\begin{array}{ccccc}
